...
桌面端首次进入只显示合上的旅行手记封面，点击后由封面绕左侧书脊翻开并进入双页阅读；返回首页则合上整本书。展开态使用等宽、无间隙的双页布局，纸页分别滚动，章节书签改为书口侧签。路线页左页承担搜索、年份和记录索引，右页先展示最近抵达、旅行概览和新增入口，再向下提供高级筛选；个人主页使用开放式档案行和统计分隔线，避免重复卡片嵌套。地图、筛选、档案、地点、日记详情和媒体附件都沿用同一纸张轮廓，日记正文不再通过独立暗幕弹层展示。桌面书本外层用 overflow: clip 限制翻页纸带的绘制溢出，避免透明变换区域撑大根滚动区；手机端使用普通单列文档流和紧凑的粘性导航，不强制保留 3D 书体，合盖页则单独固定在一个视口内。

章节、地点、日记和附件切换先保留不可交互且对辅助技术隐藏的旧页副本，再同步渲染新内容，按书签章节和篇章前后关系决定方向：向后阅读从右下角向左翻，返回前章或上一篇从左下角向右翻。斜向纸带数量会按页宽与硬件并发能力在 12、15、18 条之间调整，让圆柱曲面的弯曲边界从外侧下角连续向内推进，逐渐转为与书脊平行；正反纸面共享曲面端点和采样坐标，并通过 Web Animations 驱动合成变换。纸背增加纸纹与透光差异，遮光随曲率变化，带模糊的接触阴影跟随斜向弯曲边界，位于静态底页之上、翻动纸面之下。屏外记录在副本中替换为等高占位；旧对页保留至纸张落稳；动画在首帧栅格化完成后启动。同章节的筛选和搜索更新直接渲染，不复制纸页。所有动画与副本在连续导航和窗口尺寸变化时一并清理。翻页期间提升整个书页层级，使纸张遮住书签、包角、书脊和静态控件；真实底页暂时设为 inert，避免键盘和鼠标穿透纸面，清理动画时恢复。窄屏使用单页轻量动效，系统启用减少动态效果时直接切换内容。
