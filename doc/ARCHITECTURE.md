...
桌面端首次进入只显示合上的旅行手记封面，点击后由封面绕左侧书脊翻开并进入双页阅读；返回首页则合上整本书。展开态使用等宽、无间隙的双页布局，纸页分别滚动，章节书签改为书口侧签。地图、筛选、档案、地点、日记详情和媒体附件都沿用同一纸张轮廓，日记正文不再通过独立暗幕弹层展示。桌面书本外层用 overflow: clip 限制翻页纸带的绘制溢出，避免透明变换区域撑大根滚动区；手机端使用普通单列文档流，不强制保留 3D 书体。
