...
本地全量数据导出遍历普通文件并把 `data/` 作为 ZIP 根目录，通过仅允许本机访问的 HTTP 下载地址返回。GitHub Pages 部署阶段调用 `scripts/build-data-backup.js` 生成同结构静态 ZIP，避免浏览器 Blob 地址被外部下载工具接管后得到空文件。导入先用当前 `data/password.json` 授权，服务端在取得数据锁后复核 `X-Travel-Current-Password`，再校验 ZIP 路径与跨平台大小写冲突、索引 JSON、记录字段和日期、正文路径与 UTF-8 内容、照片引用及密码配置；备份内密码只决定导入后的访问密码，不用于授权覆盖当前数据。备份缺少密码时，服务端以 `IMPORT_PASSWORD_REQUIRED` 响应要求前端完成两次 6 位数字输入，再把配置补入暂存数据。所有文件写入项目内临时目录后才通过 `rename` 替换，任何密码错误、数据校验失败、取消设置或写入失败都会保留原目录并清理临时内容。
