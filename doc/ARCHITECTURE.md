...
桌面端使用等宽、无间隙的双页书本布局，纸页分别滚动。章节切换先保留不可交互且对辅助技术隐藏的旧页副本，再同步渲染新内容，统一从左页左下角向右掀起，再使用 12 个连续窄片沿纸张切线积分形成曲面，并通过 Web Animations 驱动合成变换；遮光层只动画透明度，屏外记录在副本中替换为等高占位；旧对页保留至纸张落稳；动画在首帧栅格化完成后启动。同章节的筛选和搜索更新直接渲染，不复制纸页。所有动画与副本在连续导航和窗口尺寸变化时一并清理。翻页期间提升整个书页层级，使纸张遮住书签、包角、书脊和静态控件。日记详情使用独立阅读纸，打开、换篇和关闭复用背景 DOM、滚动位置与返回焦点；只有从附件页返回、直接访问详情或数据更新后才重新渲染背景，详情不触发整书翻动或背景模糊。窄屏使用单页轻量动效，系统启用减少动态效果时直接切换内容。
