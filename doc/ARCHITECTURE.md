...
桌面端使用等宽、无间隙的双页书本布局，纸页分别滚动。左右页与页边高光共用圆角变量，地图、筛选和档案页面沿用同一纸张轮廓。桌面书本外层用 overflow: clip 限制翻页纸带的绘制溢出，避免透明变换区域撑大根滚动区；手机端及矮窗口原有的文档滚动保持可用。章节切换先保留不可交互且对辅助技术隐藏的旧页副本，再同步渲染新内容，按书签章节顺序决定方向：向后阅读从右下角向左翻，返回前章从左下角向右翻。使用 18 条斜向纸带，让圆柱曲面的弯曲边界从外侧下角连续向内推进，逐渐转为与书脊平行；正反纸面共享曲面端点和采样坐标，并通过 Web Animations 驱动合成变换，遮光随曲率变化，接触阴影跟随斜向弯曲边界，位于静态底页之上、翻动纸面之下。屏外记录在副本中替换为等高占位；旧对页保留至纸张落稳；动画在首帧栅格化完成后启动。同章节的筛选和搜索更新直接渲染，不复制纸页。所有动画与副本在连续导航和窗口尺寸变化时一并清理。翻页期间提升整个书页层级，使纸张遮住书签、包角、书脊和静态控件；真实底页暂时设为 inert，避免键盘和鼠标穿透纸面，清理动画时恢复。日记详情使用独立阅读纸，打开、换篇和关闭复用背景 DOM、滚动位置与返回焦点；只有从附件页返回、直接访问详情或数据更新后才重新渲染背景，详情不触发整书翻动或背景模糊。窄屏使用单页轻量动效，系统启用减少动态效果时直接切换内容。
