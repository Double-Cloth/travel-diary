...
桌面端首次进入只显示合上的旅行手记封面。`css/11-skeuomorphic-book.css` 统一皮革、黄铜、织物书签、凹陷输入框与纸芯材质，沿用本地纸纹和背景，不增加运行依赖。封面宽度始终对应展开书本的一半；开书时用一张与真实封面同外观的轻量贴图遮挡新书页，等连续帧稳定后以 240ms 淡出，让首次绘制发生在封面静止期间。合书时贴图淡入后再渲染封面路由，避免正文提前消失；过渡完成或被导航、缩放打断时统一清理遮罩、恢复 aria-hidden 和可见面的键盘焦点。桌面书本外层用 overflow: clip 限制翻页纸带的绘制溢出，纸页分别滚动，章节书签位于书口。路线、个人主页、地点、日记和附件沿用同一纸张轮廓。手机端采用单列纸页、皮革粘性导航和同样的轻量封面过渡，合盖页固定在一个视口内；启用减少动态效果时直接切换。
...
这些效果仅使用 CSS 和仓库内已有的图片、纹理与字体。书页把本地地图纸素材作为淡背景叠在纸纹之下，个人主页使用对应档案素材。运行时不加载 CDN、远程字体或远程图标；更换设计参考时也必须保持离线可用。

路线页把搜索与“筛选与排序”入口放在左页同一工作区，右页只保留筛选内容；手机端点击入口直接打开筛选夹层。下拉菜单按书页和视口的可见空间确定宽度、高度与展开方向。年份由左页书签统一控制，夹层不再重复提供年份选择。清除筛选与排序只在存在可清除状态时出现。个人主页将数据备份与清空数据分开，避免破坏性操作和导入导出并列。
...
日记详情采用信息页与正文页分工：左页显示返回入口、日期、标题、地点、带标题的相邻篇目导航和原生 details 记录管理菜单，不再自动摘录正文；右页只渲染一次 Markdown 内容，有附件时才显示媒体袖套。正文页页眉显示当前筛选范围内的篇目位置，手机端也显示左页的相邻篇目导航。新增与头像工具置于桌面书本外侧，避免与阅读页眉冲突。
...
- `css/09-book-experience.css`：合盖封面、书口侧签、书内日记详情与纸张材质增强。
- `css/10-refined-ui.css`：最终视觉系统覆盖层，统一书本比例、书签与工具定位、路线概览、开放式档案列表、控件层级和移动端阅读器布局。
- `css/11-skeuomorphic-book.css`：实体材质细节与位图封面、统一页边距、控件反馈及手机阅读排布。首页开合由 `app.js` 在静态封面遮罩下绘制新页，再以 240ms 淡入或淡出。
