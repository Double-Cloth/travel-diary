...
| 全部数据导入失败 | 确认 ZIP 来自本应用，记录字段、日期和路径有效，并包含 `data/travel_data.json` 以及索引引用的全部正文和照片；若包含 `data/password.json`，密码须为 6 位数字。 |
| 导入时要求设置密码 | 备份未包含 `data/password.json`；连续两次输入相同的 6 位数字即可随导入创建配置，取消则不会修改当前数据。 |
