...
1. 在项目根目录运行 `npm start`，打开终端显示的 `http://localhost:端口`。
2. 点击头部「＋」或首页「新增旅行记录」。窄屏使用首页入口。
3. 填写旅行日期、国家 / 地区、城市 / 目的地和标题。一级行政区、正文、旅行标识为选填；正文支持下文列出的 Markdown 语法，标题由表单自动生成一级标题。
4. 点击「保存到项目文件」。只有服务器确认写入后才显示保存成功，同时重新加载首页与统计。
5. 检查 `git diff`，按需更新字体子集，运行 `npm test`，提交并推送内容文件以更新线上站点。

本地保存真实追加 `data/travel_data.json`，并在 `data/travel-diary/YYYY/` 创建 `YYYY-MM-DD-随机标识.md`。随机标识用于避免文件名冲突，同一草稿重复提交不会重复添加。日记标题与目的地不会作为可执行内容或文件路径处理。页面暂不上传照片，新记录的 `photos` 为空；照片请按下文规范放入 `data/photos/` 后维护相应字段。

关闭新增窗口后，未保存内容仍留在当前页面，再次打开可继续填写；刷新或离开页面会丢失这份内存草稿，应先下载。浏览器通常会对未保存内容提示离开确认，但不能以此替代备份。草稿也可以在字段未填完整时下载。

### GitHub Pages、静态托管与保存失败

GitHub Pages 不运行本项目的 Node.js 写入服务，无法改写仓库。页面会显示只读提示并禁用保存按钮，仍可离线编辑已加载的表单、下载草稿；不使用 `localStorage` 冒充保存，也不会把未写入文件的内容显示为正式记录。

1. 点击「下载草稿」，确认浏览器已下载 `travel-diary-draft-随机标识.json`。
2. 在本机检出的项目目录运行 `npm start`。
3. 打开新增窗口，点击「导入草稿」，选择下载的 JSON 文件，检查内容后点击「保存到项目文件」。已有未保存内容的窗口不会被导入覆盖，请先保存，或下载并刷新页面后再导入。
4. 提交并推送生成的日记与更新后的索引，等待 GitHub Pages 部署。

草稿是携带表单字段的中间文件，**不能直接替换 `data/travel_data.json`**。下载、导入草稿都不等于保存或发布。离线使用指在本机启动项目后使用仓库内资源；线上页面并未新增 Service Worker 缓存，不保证断网后首次打开或刷新仍可加载。

本机服务器不可用、磁盘权限不足、索引格式损坏或写入冲突时，会保留表单内容并显示错误，可下载后重试。若请求超时，写入可能已经完成，重试同一份未修改草稿会核对文件并返回已保存；不要为重试另建一份记录。若此前已保存的草稿内容被修改，系统会拒绝覆盖。
