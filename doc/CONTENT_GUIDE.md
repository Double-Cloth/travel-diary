...
| 旅行正文 | 日记标题、Markdown 编辑、渲染预览和完整源码。 |
| 文件与照片 | 正文文件路径、照片目录和有序文件列表；点击区域标题展开。 |

日期、国家 / 地区、目的地和日记标题为必填项。国家显示名称随国家选择填入，允许自定义；留空时使用国家目录名称。其他字段为选填，字段定义见下文。

单行字段不超过 200 字符，正文不超过 100000 字符；照片列表最多 1000 项，每个文件名不超过 200 字符。JSON 草稿文件及保存请求的大小上限为 512 KB。
...
| 操作 | 输出或行为 |
| --- | --- |
| 编辑 | 编辑 Markdown 正文；日记标题由独立字段管理。 |
| 预览 | 使用日记详情相同的解析器显示标题与正文。 |
| 源码 | 只读显示完整 Markdown，包含自动生成的一级标题，与正文导出内容一致。 |
| 导出正文 .md | 下载独立 Markdown 文件，包含标题和正文，不包含地点、旅行标识或照片配置。 |
| 导出草稿 | 下载包含全部表单字段的 JSON 草稿，可用于备份和跨环境继续编辑。 |
| 导入草稿 | 读取 JSON 草稿并填入表单；支持 v1 与 v2 格式。导入后需另行保存。 |

编辑、预览和源码切换保留输入内容；键盘方向键、Home 和 End 可切换正文标签。源码与预览仅用于查看，正文修改在「编辑」视图进行。Markdown 支持范围见下文「Markdown 写法」。

JSON 草稿与旅行索引的结构不同，不能用草稿替换 `data/travel_data.json`。草稿不嵌入照片文件。导入时保留全部已支持字段和照片顺序，旧版草稿缺失的可选字段采用空值。存在未保存内容时，导入操作不会覆盖当前草稿。
...
照片目录限定在 `data/photos/` 下，允许多级目录。照片列表每行填写一个文件名，不含目录部分，显示顺序与填写顺序一致。目录名和文件名使用英文字母、数字、连字符、下划线或点，不使用系统保留名称。照片须提前放入项目对应目录；保存时检查文件是否存在，编辑器不负责上传照片。未填写照片列表时允许仅配置目录。
...
只读环境下可导出 Markdown 正文或 JSON 草稿。需要保存完整记录时，将 JSON 草稿导入本机编辑器，核对内容后保存。草稿导出不修改站点内容，也不触发发布。
