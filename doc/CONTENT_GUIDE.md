...
单行字段不超过 200 字符，正文不超过 100000 字符；照片列表最多 1000 项，每个文件名不超过 200 字符。上传图片支持 JPEG、PNG、GIF 和 WebP，最多 20 张，单张上限 10 MB、合计上限 30 MB。包含图片的 JSON 草稿和保存请求上限为 44 MB。
...
| 导出正文 .md | 下载独立 Markdown 文件，包含标题和正文，不包含地点、旅行标识或照片配置。 |
...
```text
data/travel-diary/YYYY/YYYY-MM-DD-随机标识.md
```

自定义路径必须保留与旅行日期一致的年份目录和日期前缀，例如 `data/travel-diary/2026/2026-09-11-suzhou.md`。文件名仅允许英文字母、数字、连字符、下划线和点，禁止路径穿越和系统保留名称。保存不会覆盖已有正文；相同路径、元数据和正文的重复提交返回已保存状态。

「上传照片」支持多选，也可将图片拖入上传区域。缩略图下方提供前移、后移和移除操作。选图阶段仅在浏览器中保留内容；保存旅行记录时才写入磁盘。

上传照片自动存放到 `data/photos/YYYY-MM-DD-草稿标识/`，文件名使用独立标识及实际图片格式扩展名，不受原始文件名重复或中文名称影响。记录中的 `photo_folder` 和 `photos` 自动填写，展示顺序与缩略图顺序一致。上传保留原始图片字节，不进行压缩或裁剪。

「文件设置」中的照片目录和列表用于引用项目中已经存在的文件，照片目录限定在 `data/photos/` 下。存在新上传照片时，已有照片会复制到新目录并排在上传照片之前，原文件保持不变；复制照片与上传照片的总大小同样受 30 MB 上限约束。仅引用已有照片时不创建新目录。
...
只读环境下可选择照片、编辑正文并导出 Markdown 或包含上传图片的 JSON 草稿。需要保存完整记录时，将 JSON 草稿导入本机编辑器，核对内容后保存。草稿导出不修改站点内容，也不触发发布。
