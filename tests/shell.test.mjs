...
test('自定义下拉框在点击而非按下时选择，保留移动端滑动能力', () => {
    ...
    assert.match(indexHtml, /js\/app\.js\?v=20260921-book-bounds-v6/);
    assert.match(indexHtml, /css\/journal\.css\?v=20260921-book-bounds-v6/);
...
test('自定义下拉框初始方向和打开后的箭头都随弹出方向变化', () => {
    ...
    assert.match(journalEntryCss, /08-custom-select\.css\?v=20260921-book-bounds-v6/);
