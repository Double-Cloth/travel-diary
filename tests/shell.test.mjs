...
test('自定义下拉框在点击而非按下时选择，保留移动端滑动能力', () => {
    assert.match(customSelectJs, /menu\.addEventListener\('pointerdown',[\s\S]*?event\.pointerType === 'mouse'[\s\S]*?event\.preventDefault\(\);[\s\S]*?optionPointer = null;[\s\S]*?return;/);
    ...
    assert.match(recordEditorJs, /dialog\.addEventListener\('pointerdown',[\s\S]*?event\.pointerType === 'mouse'[\s\S]*?event\.preventDefault\(\);[\s\S]*?autocompletePointer = null;[\s\S]*?return;/);
    ...
    assert.match(indexHtml, /js\/app\.js\?v=20260923-skeuomorphic-v2/);
    assert.match(indexHtml, /css\/journal\.css\?v=20260923-skeuomorphic-v2/);
...
test('自定义下拉框初始方向和打开后的箭头都随弹出方向变化', () => {
    ...
    assert.match(journalEntryCss, /08-custom-select\.css\?v=20260922-mobile-layout-v1/);
...
test('新增和修改记录使用独立编辑器实例并生成互不重复的控件 ID', () => {
    ...
    assert.match(appJs, /const requestEditAuthorization = createPasswordGate\(\(capability, record\) => openUpdateEditor\(record, capability\),/);
    assert.doesNotMatch(appJs, /pendingEditRecord/);
    assert.match(recordPasswordJs, /await onVerified\(capability, verifiedContext\)/);
...
test('全部数据导入成功后使用站内结果弹窗而不是状态文字', () => {
    ...
    assert.match(dataTransferJs, /await onImported\(\);[\s\S]*setStatus\(''\);\s*showImportSuccess\(importSuccessMessage\(result\)\);/);
...
test('章节书签保留语义导航并贴合书页上沿', () => {
    assert.match(indexHtml, /<nav class="chapter-tabs"[^>]*aria-label="章节导航"/);
    assert.match(journalCss, /\.journal-spine\s*{[^}]*align-items: center;/);
    assert.match(journalCss, /\.chapter-tabs\s*{[^}]*align-self: end;/);
...
test('移动端视频查看器同排显示播放和进度并可折叠其余控件', () => {
    ...
    assert.match(appJs, /class="video-viewer-more-icon"[^>]*><path d="M1 6\.5 6 1\.5l5 5"><\/path>/);
