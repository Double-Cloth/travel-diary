...
test('随机路线图按屏幕宽度提供不同抽取区间', () => {
    ...
    assert.deepEqual(getRouteMapCountRange(640, 6), { min: 3, max: 4 });
    assert.deepEqual(getRouteMapCountRange(900, 6), { min: 4, max: 5 });
    assert.deepEqual(getRouteMapCountRange(1280, 6), { min: 5, max: 6 });
});
