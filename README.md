...
个人主页的「数据备份」可导出整个 `data/`，本机模式还可从应用生成的 ZIP 完整导入。导出与导入都需要先使用当前访问密码授权；导入请求还会由本地服务复核当前密码，再校验 ZIP、旅行记录、正文、照片引用和密码配置。若备份未包含密码或密码值为空字符串，则会要求连续输入两次新的 6 位数字密码；其他非 6 位数字格式会拒绝导入。导入会替换现有个人数据，操作前请先备份。详细用法、字段定义和环境限制见 [内容维护指南](doc/CONTENT_GUIDE.md#页面编辑器)。
