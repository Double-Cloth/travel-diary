...
个人主页的「数据备份」可导出整个 `data/`，本机模式还可从应用生成的 ZIP 完整导入。导出需要使用同一访问密码；导入前会校验 ZIP、旅行记录、正文、照片引用和密码配置，若备份未包含密码则会要求连续输入两次新的 6 位数字密码。导入会替换现有个人数据，操作前请先备份。详细用法、字段定义和环境限制见 [内容维护指南](doc/CONTENT_GUIDE.md#页面编辑器)。
