...
GitHub Pages 等静态页面不能写回仓库，会显示只读提示。可通过「导出草稿」下载 ZIP：`draft.json` 保存字段，上传图片作为独立文件存放，不会以内嵌 Base64 写入 JSON；之后可在本机「导入草稿」并保存。个人主页的「数据备份」可在静态页面导出旅行索引引用的 `data/` 文件；本机 `npm start` 页面会直接打包整个 `data/` 目录，并支持完整导入。功能说明、字段定义和环境限制见 [内容维护指南](doc/CONTENT_GUIDE.md#页面编辑器)。
